...
from app.schemas.acta import ActaRenderRequest, ActaTemplateCreate, ActaTemplateRead
...
def render_acta_pdf(template_id: str, payload: ActaRenderRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> Response:
    ...
    safe_name = "".join(character if character.isascii() and (character.isalnum() or character in "-_") else "_" for character in template.name).strip("_") or "acta"
    return Response(
        content=pdf_bytes,
