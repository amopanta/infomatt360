...
def seed(db: Session) -> dict[str, str]:
    ...
    role = upsert(
        ...
        permissions="projects.read,users.admin,identity.users.manage,records.read,records.write,records.review,records.coordinate,records.approve,reports.export,gis.read,builder.write,messages.write,messages.read,integrations.api_keys.manage,organizations.manage,organizations.branding.manage,organizations.tenant_clean,backups.manage,erp.manage,integrations.donor_sync.manage,ai.audit.manage,support.tickets.manage",
    )
    project = upsert(
        db,
        Project,
