...
flowchart TB
    ...
    WEB --> NGINX
    NGINX -->|"fetch /api/v1/*"| API
    DESK -->|"HTTP local + fetch remoto"| API
    PWA --> NGINX
...
Fuente de los componentes y sus relaciones: `docker-compose.production.example.yml` (servicios `postgres`, `redis`, `backend`, `worker-bulk`, `worker-scheduler`, `frontend`), `deploy/backend.Dockerfile`, `deploy/frontend.Dockerfile`, `backend/app/main.py` (middlewares y router), `backend/app/core/config.py` (integraciones opcionales).
...
La receta de referencia (`docker-compose.production.example.yml`) define 6 servicios:
...
| `backend` | build `deploy/backend.Dockerfile` | `8000:8000` | `postgres` (healthy), `redis` (healthy) | `GET /api/v1/health/ready` cada 30s |
| `worker-bulk` | misma imagen que `backend` | — (sin puerto, proceso batch) | `postgres` (healthy), `redis` (healthy) | sin healthcheck definido |
| `worker-scheduler` | misma imagen que `backend` | — (sin puerto, proceso batch) | `postgres` (healthy), `redis` (healthy) | sin healthcheck definido |
| `frontend` | build `deploy/frontend.Dockerfile` (nginx) | `8080:80` | `backend` (healthy) | sin healthcheck definido |

**Volúmenes persistentes:** `postgres_data`, `redis_data`, `uploads_data` (montado en `backend`, `worker-bulk` y `worker-scheduler` como `/var/lib/infomatt360/uploads`).

**Topología de red:** todos los servicios comparten la red default de Docker Compose generada automáticamente; solo `backend` (8000) y `frontend` (8080) publican puertos al host. `postgres` y `redis` **no** son accesibles fuera de la red interna de Compose en esta receta.

**Reverse proxy / TLS / dominio:** **no está incluido en la receta**. `docs/61_DESPLIEGUE_PRODUCCION_REFERENCIA.md` es explícito: *"Estos archivos son una base de referencia. Antes de usarlos en producción real deben conectarse a dominio, TLS/HTTPS, backups, monitoreo y gestión de secretos del proveedor de infraestructura."* → **Pendiente de confirmar por infraestructura.**

**Proveedor cloud / VPS / on-premise, escalado horizontal, número de réplicas, balanceador de carga real:** sin evidencia en el repositorio → **Pendiente de confirmar por infraestructura.**
...
| Imagen | Base | Notas |
|---|---|---|
| `backend` | `python:3.13-slim` | Instala `backend/requirements.txt`, expone `8000`, arranca con `uvicorn app.main:app --host 0.0.0.0 --port 8000` (`deploy/backend.Dockerfile`) |
...
|---|---|---|
| `8000` | Backend (Uvicorn) | Dev local (`README.md`) y contenedor de producción |
...
| `API_RATE_LIMIT_TRUSTED_PROXY_IPS` | vacío | IP(s) del proxy real | Recomendado (si no, `X-Forwarded-For` se ignora) |
...
2. `docker compose ... up -d --build backend frontend worker-bulk worker-scheduler`.
3. Validar con `check-health.cmd`.
4. Revisar logs (`docker compose ... logs backend` / `worker-bulk` / `worker-scheduler`).
