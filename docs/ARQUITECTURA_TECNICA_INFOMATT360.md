...
Fuente de los componentes y sus relaciones: `docker-compose.production.example.yml` (servicios `postgres`, `redis`, `backend`, `worker-bulk`, `frontend`), `deploy/backend.Dockerfile`, `deploy/frontend.Dockerfile`, `backend/app/main.py` (middlewares y router), `backend/app/core/config.py` (integraciones opcionales).
...
La receta de referencia (`docker-compose.production.example.yml`) define 5 servicios:
...
**Volúmenes persistentes:** `postgres_data`, `redis_data`, `uploads_data` (montado en `backend` y `worker-bulk` como `/var/lib/infomatt360/uploads`).
...
`worker-bulk` reutiliza la imagen `backend` con un comando distinto (ver sección 8) — no tiene Dockerfile propio.
...
2. `docker compose ... up -d --build backend frontend worker-bulk`.
3. Validar con `check-health.cmd`.
4. Revisar logs (`docker compose ... logs backend` / `worker-bulk`).

**Si falla después de migraciones de base de datos** (ruta más delicada):
1. Detener tráfico / modo mantenimiento.
2. Detener `worker-bulk` (evitar escrituras nuevas).
