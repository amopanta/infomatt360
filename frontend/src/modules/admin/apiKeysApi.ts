...
export async function createApiKey(payload: { projectId: string; name: string; permissions: string[]; rateLimitProfile: string }): Promise<ApiKeyCreateResponse> {
  const response = await fetch(`${API_BASE_URL}/api-keys/`, {
    method: 'POST',
    headers: jsonHeaders(),
    body: JSON.stringify({ project_id: payload.projectId, name: payload.name, permissions: payload.permissions, rate_limit_profile: payload.rateLimitProfile }),
  });
