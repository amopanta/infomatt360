...
import { blankCommittee, getCatalog, issueReportLink, listCatalog, listReportLinks, revokeReportLink, saveCatalog } from './catalogApi';
...
export function ReportCatalogApp() {
  ...
  function startNew() {
    ...
    setDraft({ project_id: projectId, name: '', description: '', report_kind: 'indicators', committee: blankCommittee(), starts_at: null, ends_at: null, indicators: [blankIndicator(templates[0]?.id)] });
    setEditing(true);
  }
  ...
  function startEdit() {
    ...
    if (!report) return;
    setDraft({ project_id: report.project_id, name: report.name, description: report.description, report_kind: report.report_kind ?? 'indicators', committee: report.committee ?? blankCommittee(), starts_at: localDateInput(report.starts_at), ends_at: localDateInput(report.ends_at), indicators: report.indicators.map((item) => ({ ...item })) });
    setEditing(true);
  }
