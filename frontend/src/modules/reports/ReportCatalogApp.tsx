...
import { getCatalog, issueReportLink, listCatalog, listReportLinks, revokeReportLink, saveCatalog } from './catalogApi';
import type { CatalogReport, CatalogReportConfig, CatalogResult, IndicatorDefinition, ReportLink } from './catalogApi';

function blankIndicator(templateId = ''): IndicatorDefinition {
  return { title: '', source_mode: 'automatic', template_id: templateId || null, value_field: null, aggregation: 'count', goal: 0, manual_actual: null, unit: '', municipality_field: null, view_kind: 'progress' };
}
...
export function ReportCatalogApp() {
  ...
  useEffect(() => {
    const templateIds = new Set([...templates.map((item) => item.id), ...(draft?.indicators.map((item) => item.template_id) ?? [])]);
  ...
  function startNew() {
    ...
    setDraft({ project_id: projectId, name: '', description: '', starts_at: null, ends_at: null, indicators: [blankIndicator(templates[0]?.id)] });
    setEditing(true);
  }
  ...
  function startEdit() {
    ...
    if (!report) return;
    setDraft({ project_id: report.project_id, name: report.name, description: report.description, starts_at: localDateInput(report.starts_at), ends_at: localDateInput(report.ends_at), indicators: report.indicators.map((item) => ({ ...item })) });
    setEditing(true);
  }
  ...
  return <AppShell title="Reportes"><main className="reports-shell reports-studio report-catalog">
    <header className="reports-header"><div><h2>Reportes personalizados</h2><p>Vincula formularios, variables, metas y municipios.</p></div><div className="reports-actions"><a href="/reports">← Tablero</a>{canEdit && <button type="button" onClick={startNew}>+ Crear reporte</button>}</div></header>
    {message && <p role="alert" className="catalog-message">{message}</p>}
    <div className="catalog-layout"><aside className="catalog-list"><h3>Mis reportes</h3>{reports.length ? reports.map((item) => <button type="button" className={selectedId === item.id ? 'active' : ''} key={item.id} onClick={() => void open(item.id)}><strong>{item.name}</strong><small>{item.indicators.length} indicador(es)</small></button>) : <p>Aún no hay reportes. Crea uno para relacionar tus formularios con una meta.</p>}</aside>
